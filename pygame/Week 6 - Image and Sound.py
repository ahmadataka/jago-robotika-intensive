...
# Load images
player_image = pygame.image.load("player.png").convert_alpha()  # Convert to handle transparency
enemy_image = pygame.image.load("enemy.png").convert_alpha()    # Convert to handle transparency
background_image = pygame.image.load("background.jpeg").convert()  # Optional for non-transparent background

# Resize images to match the required size
player_image = pygame.transform.scale(player_image, (50, 50))  # Resize player image
enemy_image = pygame.transform.scale(enemy_image, (50, 50))    # Resize enemy image
background_image = pygame.transform.scale(background_image, (800, 600))  # Resize background image
...
# pygame.mixer.music.load("background_music.mp3")  # Replace with the path to your background music file
...
while running:
    ...
    # Clear the screen with the background image (before drawing new images)
    screen.fill((0, 0, 0))  # Clear the screen with black fill first
    screen.blit(background_image, (0, 0))  # Now draw the background image

    # Draw the player (using the image instead of a rectangle)
    screen.blit(player_image, (player_x, player_y))  # Draw player at the current position

    # Draw the enemy (using the image instead of a rectangle)
    screen.blit(enemy_image, (enemy_x, enemy_y))  # Draw enemy at the current position

    # Draw the goal (as a simple rectangle or image)
    pygame.draw.rect(screen, (0, 0, 255), (goal_x, goal_y, goal_width, goal_height))  # Draw goal as a rectangle
